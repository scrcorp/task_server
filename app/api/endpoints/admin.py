...
from app.core.dependencies import (
    get_user_repo, get_org_repo, get_template_repo,
    get_task_repo, get_feedback_repo, get_notice_repo,
)
...
async def list_brands():
    service = _get_admin_service()
    return await service.org_repo.list_brands()

@router.post("/brands", status_code=201)
async def create_brand(body: BrandCreateRequest):
    service = _get_admin_service()
    return await service.org_repo.create_brand({"name": body.name})

@router.patch("/brands/{brand_id}")
async def update_brand(brand_id: str, body: BrandCreateRequest):
    service = _get_admin_service()
    return await service.org_repo.update_brand(brand_id, {"name": body.name})

@router.delete("/brands/{brand_id}")
async def delete_brand(brand_id: str):
    service = _get_admin_service()
    await service.org_repo.delete_brand(brand_id)
...
@router.get("/branches")
async def list_branches(brand_id: Optional[str] = None):
    service = _get_admin_service()
    return await service.org_repo.list_branches(brand_id)

@router.post("/branches", status_code=201)
async def create_branch(body: BranchCreateRequest):
    service = _get_admin_service()
    return await service.org_repo.create_branch(body.model_dump())

@router.delete("/branches/{branch_id}")
async def delete_branch(branch_id: str):
    service = _get_admin_service()
    await service.org_repo.delete_branch(branch_id)
...
@router.get("/groups")
async def list_groups(branch_id: Optional[str] = None):
    service = _get_admin_service()
    return await service.org_repo.list_groups(branch_id)

@router.post("/groups", status_code=201)
async def create_group(body: GroupCreateRequest):
    service = _get_admin_service()
    return await service.org_repo.create_group(body.model_dump())

@router.delete("/groups/{group_id}")
async def delete_group(group_id: str):
    service = _get_admin_service()
    await service.org_repo.delete_group(group_id)
...
async def list_templates(brand_id: Optional[str] = None, group_id: Optional[str] = None):
    service = _get_admin_service()
    return await service.template_repo.list_templates(brand_id, group_id)
...
async def list_feedbacks(target_user_id: Optional[str] = None, task_id: Optional[str] = None):
    service = _get_admin_service()
    return await service.feedback_repo.list_feedbacks({
...
async def create_feedback(data: dict):
    service = _get_admin_service()
    return await service.feedback_repo.create_feedback(data)
